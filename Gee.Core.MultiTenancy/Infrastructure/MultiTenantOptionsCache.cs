...
namespace Gee.Core.MultiTenancy.Infrastructure
{
    /// <summary>
        ...
        where TOptions : class where T : ITenantInfo
    {
        ...
        public void Clear()
        {
            var tenantId = multiTenantContextAccessor.TenantInfo?.Id;
            tenantCaches.GetOrAdd(tenantId?.ToString() ?? "no-tenant", new OptionsCache<TOptions>())
                 .Clear();
        }
        ...
        public TOptions GetOrAdd(string? name, Func<TOptions> createOptions)
        {
            ...
            var cache = tenantCaches.GetOrAdd(tenantId?.ToString() ?? "no-tenant", new OptionsCache<TOptions>());
