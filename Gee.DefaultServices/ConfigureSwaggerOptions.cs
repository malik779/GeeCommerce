...
internal sealed class ConfigureSwaggerOptions : IConfigureOptions<SwaggerGenOptions>
{
    ...
    {
        var SwaggerOptions= _configuration.GetSection("SwaggerOptions");  
        var openApi = SwaggerOptions?.GetSection("OpenApi");
        var document = openApi?.GetRequiredSection("Document");
        var info = new OpenApiInfo()
        {
            Title = document?.GetRequiredValue("Title") ?? "Swagger UI",
            Version = description.ApiVersion.ToString(),
            Description = BuildDescription(description, (document?.GetRequiredValue("Description")?? "Service HTTP API")),
        };
    ...
    private void ConfigureAuthorization(SwaggerGenOptions options)
    {
        ...
        var scopes = identitySection.GetRequiredSection("Scopes").GetChildren().ToDictionary(p => p.Key, p => p.Value); // Use GetValue<string> for direct value retrieval
        ...
        // Add security definitions for O Auth2
